...
load("results/indices for assessment/summer flounder/mitigation_1_WEA_distance/MIT.1.ALB_ratio.Rdata")
load("results/indices for assessment/summer flounder/mitigation_1_WEA_distance/MIT.1.BIG_ratio.Rdata")
...
BIG_fall <- data$dat$IAA_mats[[26]][28:41,c(2,4:11)]
    ...
    ## 2.3 apply the ratio to estimate Wind Energy Excluded indices (MIT.1) ----

# note: be careful with the round and -999 values, need to be very consistent with the original indices format 

ALB_spring_MIT.1 <- round(ALB_spring * ALB_MIT.1_SPRING_ratio, 2)
ALB_fall_MIT.1 <-   round(ALB_fall *   ALB_MIT.1_FALL_ratio, 2)

BIG_spring_MIT.1 <- round(BIG_spring * BIG_MIT.1_SPRING_ratio)
BIG_spring_MIT.1[c(12),] <- -999

BIG_fall_MIT.1 <-   round(BIG_fall *   BIG_MIT.1_FALL_ratio)
BIG_fall_MIT.1[c(9, 12),] <- -999
...
data$dat$IAA_mats[[2]][1:27,c(2,4:11)] <- ALB_spring_MIT.1

# ALB fall (1982-2008)
data$dat$IAA_mats[[3]][1:27,c(2,4:11)] <- ALB_fall_MIT.1

# BIG spring (2009-2022)
data$dat$IAA_mats[[25]][28:41,c(2,4:11)] <- BIG_spring_MIT.1

# BIG fall (2009-2022)
data$dat$IAA_mats[[26]][28:41,c(2,4:11)] <- BIG_fall_MIT.1

WriteASAP3DatFile("assessment model/summer flounder/ASAP input data/ASAP3_MTA2023_MIT.1_BTS.DAT", data, 
                  header.text = "Summer Flounder 2023: BTS indices using MIT.1 dataset (Sun 2024) \n \n \n ") 
...
wd <- "assessment model/summer flounder/ASAP/MIT.1 model/"
...
output <- read.csv("results/stock assessment/summer flounder/ASAP_summary_ASAP3_MTA2023_MIT.1_BTS.csv") %>%
  add_column(Scenario = "MIT.1 Model") %>%
  write.csv("results/stock assessment/summer flounder/ASAP_summary_ASAP3_MTA2023_MIT.1_BTS.csv", row.names = FALSE)
