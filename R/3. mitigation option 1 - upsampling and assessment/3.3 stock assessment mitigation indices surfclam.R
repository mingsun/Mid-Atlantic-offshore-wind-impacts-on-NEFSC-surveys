...
load("results/indices for assessment/surfclam/mitigation_1_WEA_distance/MIT.1.RDtrendS_ratio.Rdata")
...
load("results/indices for assessment/surfclam/mitigation_1_WEA_distance//MIT.1.RDscaleS_ratio.Rdata")
...
load("results/indices for assessment/surfclam/mitigation_1_WEA_distance/MIT.1.MCDS_ratio.Rdata")
...
## 2.4 save the MIT.1 input into a data.ss ----


SS_writedat(datlist = data, outfile = "assessment model/surfclam/MIT.1 model/data.ss", overwrite = TRUE)
...
run_dir_MIT.1 <- "assessment model/surfclam/MIT.1 model/"

r4ss::run(dir = run_dir_MIT.1) # skip any folders that already contain a "Report.sso" file
...
SSB_MIT.1.MSY <- 1/2 * max(SSB_MIT.1_df$Value)
SSB_MIT.1.threshold <- 1/2 * SSB_MIT.1.MSY
...
write.csv(SSB_MIT.1_df, "results/stock assessment/surfclam/SSB_MIT.1.csv")
...
GetFref(rlst = assessment_results_MIT.1, rhoF = rho)

F_MIT.1_df$Ratio <- F_MIT.1_df$F_weighted/0.1526799
F_MIT.1_df$Ratio.SD <- F_MIT.1_df$F_weighted_std/0.1526799
F_MIT.1_df$Scenario <- "MIT.1 Model"


write.csv(F_MIT.1_df, "results/stock assessment/surfclam/F_MIT.1.csv")
...
write.csv(ABC_df, "results/stock assessment/surfclam/ABC_MIT.1.csv", row.names = FALSE)
