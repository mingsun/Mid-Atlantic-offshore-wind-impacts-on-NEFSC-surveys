...
WEE_AI <- read.csv("results/stratified.mean.indices/longfin.squid/impacted.indices.csv") %>%
  add_column(MODEL = "WEE") %>%
  rename(fit = STRATIFIED_MEAN_N, lwr = lo_CI_95, upr = up_CI_95) %>%
  select(YEAR, SEASON, MODEL, fit, lwr, upr)

MIT_1_AI <- read.csv("results/indices for assessment/squid/mitigation.1.indices.csv") %>%
  add_column(MODEL = "MIT.1") %>%
  rename(fit = STRATIFIED_MEAN_N, lwr = lo_CI_95, upr = up_CI_95) %>%
  select(YEAR, SEASON, MODEL, fit, lwr, upr)

AI_df <- rbind(original_AI, impacted_AI, MIT_1_AI) 
