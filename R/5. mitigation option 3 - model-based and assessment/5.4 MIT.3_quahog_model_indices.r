...
settings <-  make_settings(n_x = c(100, 250, 500, 1000, 2000)[4], # number of knots, the more knots the longer it runs
                           ...
                           FieldConfig = c("Omega1" = 1, "Epsilon1" = 0, "Omega2" = 1, "Epsilon2" = 1), # epsilon1 is closed here based on unsuccessful fitting
...
for (i in 1:nrow(scenario_RDtrendS_df)) {
  ...
  VAST_RDtrendS <- tryCatch(
    # attempt to fit the model
    {
      VAST_RDtrendS <- fit_model(settings = settings,
                                 ...
                                 working_dir = "results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_RDtrendS")
  ...
  save(VAST_RDtrendS, file = paste0("results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_RDtrendS/VAST_RDtrendS_",
                                    scenario_RDtrendS_df$error.dist[i], ".",scenario_RDtrendS_df$link.func[i], "_model.Rdata"))
  
  write.csv(scenario_RDtrendS_df, "results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_RDtrendS/scenario_RDtrendS.csv", row.names = FALSE)
...
load("results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_RDtrendS/final model/VAST_RDtrendS_model.Rdata")


VAST_RDtrendS_data <- plot(VAST_RDtrendS, working_dir = "results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_RDtrendS/final model/figure/")
...
write.csv(VAST_RDtrendS_df, "results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_RDtrendS/final model/VAST_RDtrendS_Indices.csv", row.names = FALSE)
...
tow_RDscaleS_df <- read.csv("results/stratified.mean.indices/quahog/total.catch.by.tow.csv")%>%
...
settings <-  make_settings(n_x = c(100, 250, 500, 1000, 2000)[1], # number of knots, the more knots the longer it runs
                           purpose = "index2", # to calculate index
                           Region = "user",
                           # when region is user, then input_grid is needed be specified in the fit_model function as the extrapolation we generated earlier
                           # the setting for temporal modelign is: FieldConfig = c("Omega1" = 0, "Epsilon1" = 0, "Omega2" = 1, "Epsilon2" = 1),
...
# c(4,0) with env.variable is the optimal model, but env messed up the indices, so we ditched the env.var
# error - Delta-Lornormal
...
for (i in 1:nrow(scenario_RDscaleS_df)) {
  ...
  VAST_RDscaleS <- tryCatch(
    # attempt to fit the model
    {
      VAST_RDscaleS <- fit_model(settings = settings,
                                 ...
                                 working_dir = "results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_RDscaleS")
  ...
  save(VAST_RDscaleS, file = paste0("results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_RDscaleS/VAST_RDscaleS_",
                                    scenario_RDscaleS_df$error.dist[i], ".",scenario_RDscaleS_df$link.func[i], "_model.Rdata"))
  
  write.csv(scenario_RDscaleS_df, "results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_RDscaleS/scenario_RDscaleS.csv", row.names = FALSE)
...
load("results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_RDscaleS/final model/VAST_RDscaleS_model.Rdata")


VAST_RDscaleS_data <- plot(VAST_RDscaleS, working_dir = "results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_RDscaleS/final model/figure/", n_samples=0)
...
write.csv(VAST_RDscaleS_df, "results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_RDscaleS/final model/VAST_RDscaleS_Indices.csv", row.names = FALSE)
...
for (i in 1:nrow(scenario_MCDS_df)) {
  ...
  VAST_MCDS <- tryCatch(
    # attempt to fit the model
    {
      VAST_MCDS <- fit_model(settings = settings,
                             ...
                             working_dir = "results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_MCDS")
  ...
  save(VAST_MCDS, file = paste0("results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_MCDS/VAST_MCDS_",
                                scenario_MCDS_df$error.dist[i], ".",scenario_MCDS_df$link.func[i], "_model.Rdata"))
  
  write.csv(scenario_MCDS_df, "results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_MCDS/scenario_MCDS.csv", row.names = FALSE)
...
load("results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_MCDS/final model/VAST_MCDS_model.Rdata")


VAST_MCDS_data <- plot(VAST_MCDS, working_dir = "results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_MCDS/final model/figure/")
...
write.csv(VAST_MCDS_df, "results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_MCDS/final model/VAST_MCDS_Indices.csv", row.names = FALSE)
...
VAST_RDtrendS_df <- read.csv("results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_RDtrendS/final model/VAST_RDtrendS_Indices.csv") %>%
...
VAST_RDscaleS_df <- read.csv("results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_RDscaleS/final model/VAST_RDscaleS_Indices.csv") %>%
  select(-c(MODEL, Season)) %>%
...
VAST_MCDS_df <- read.csv("results/indices for assessment/quahog/mitigation_3_model-based indices/VAST_MCDS/final model/VAST_MCDS_Indices.csv") %>%
  select(-c(MODEL, Season)) %>%
  mutate(YEAR = c(2012, 2015)) 
