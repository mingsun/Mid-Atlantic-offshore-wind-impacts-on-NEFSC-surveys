...
for (i in 1:nrow(cor_df)) {
  ...
  temp_ref <- subset(FD_indices_df, SEASON == cor_df$SEASON.REF[i])
...
FD_CV_df <- read.csv("results/stratified.mean.indices/summer.flounder/original.biomass.indices.csv") %>%
...
CV_df <- non_FD_df %>%
  ...
  ungroup() %>%
  mutate(SCENARIO = factor(SCENARIO, levels = c("FALL", "SPRING", "ANNUAL-FALL", "ANNUAL-SPRING"))) %>%
  mutate(MODEL = factor(MODEL, levels = c("Design-based outside dataset", "Tweedie-GAM", "Delta-GAM", "Random Forest", "Artificial Neural Network", "VAST"))) %>%
  arrange(SEASON_IND) 
