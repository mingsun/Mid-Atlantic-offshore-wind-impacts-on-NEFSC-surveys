...
BART_dist_full <- dbarts::bart(x.train = full_BART_df[, c("YEAR", "LAT", "BOTTEMP", "AVGDEPTH")], y.train = full_BART_df[, c("BIOMASS")], keeptrees = TRUE)

invisible(BART_dist_full$fit$state) # very important if you want to use the models later

### predict the results and save

BART_dist_full_predict <- dbarts:::predict.bart(BART_dist_full , newdata = full_BART_df[, c("YEAR", "LAT", "BOTTEMP", "AVGDEPTH")], type = "bart")
...
BART_comp_full_dataset_df <- full_BART_df %>%
  ...
  mutate(PREDICTED_BIOMASS = apply(BART_dist_full_predict, 2, median)) %>% # extract the median as a point estimate
  mutate(PREDICTED_BIOMASS = ifelse(PREDICTED_BIOMASS < 0, 0, PREDICTED_BIOMASS))  # post-processing to ensure minimal value should be 0

## outside dataset model (transfer ability) ----

outside_BART_df <- subset(full_BART_df, OWF == "OUTSIDE")

BART_dist_outside <- dbarts::bart(x.train = outside_BART_df[, c("YEAR", "LAT", "BOTTEMP", "AVGDEPTH")], y.train = outside_BART_df[, c("BIOMASS")], keeptrees = TRUE)
  
invisible(BART_dist_outside$fit$state) # very important if you want to use the models later


### predict the results and save

BART_dist_outside_predict <- dbarts:::predict.bart(BART_dist_outside , newdata = full_BART_df[, c("YEAR", "LAT", "BOTTEMP", "AVGDEPTH")], type = "bart")
...
BART_comp_outside_dataset_df <- full_BART_df %>%
  ...
  mutate(PREDICTED_BIOMASS = apply(BART_dist_outside_predict, 2, median)) %>%
  mutate(PREDICTED_BIOMASS = ifelse(PREDICTED_BIOMASS < 0, 0, PREDICTED_BIOMASS))  # post-processing to ensure minimal value should be 0


BART_comp <- rbind(BART_comp_full_dataset_df, BART_comp_outside_dataset_df)
write.csv(BART_comp, "results/spatial model/summer.flounder/BART_abundance_comparison.csv", row.names = FALSE)

save(BART_dist_full, full_BART_df, file = "results/spatial model/summer.flounder/models objects/BART_dist_full.Rdata")
save(BART_dist_outside, full_BART_df, file = "results/spatial model/summer.flounder/models objects/BART_dist_outside.Rdata")
