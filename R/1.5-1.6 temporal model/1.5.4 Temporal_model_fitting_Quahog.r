...
write.csv(TW.GAM_temp_df, "results/temporal model/quahog/Tweedie.GAM_YEAR_Indices.csv", row.names = FALSE)

save(TW.GAM_temp_GBK, full_WG_df, file = "results/temporal model/quahog/models objects/TW.GAM_temp_GBK.Rdata")
save(TW.GAM_temp_SVAtoSNE, full_WG_df, file = "results/temporal model/quahog/models objects/TW.GAM_temp_SVAtoSNE.Rdata")
...
save(DELTA.GAM_fit_year_GBK, DELTA.GAM_PREDICTED_GBK, file = "results/temporal model/quahog/models objects/DELTA.GAM_year_effect_GAM_GBK.Rdata")
...
save(DELTA.GAM_fit_year_SVAtoSNE, DELTA.GAM_PREDICTED_SVAtoSNE, file = "results/temporal model/quahog/models objects/DELTA.GAM_year_effect_GAM_SVAtoSNE.Rdata")
...
load("results/spatial model/quahog/models objects/DELTA.S1.GAM_dist_outside.Rdata")
load("results/spatial model/quahog/models objects/DELTA.S2.GAM_dist_outside.Rdata")
...
save(DELTA.GAM_fit_year_annual, DELTA.GAM_PREDICTED_annual, file = "results/temporal model/quahog/models objects/DELTA.GAM_year_effect_GAM_annual.Rdata")
...
write.csv(DELTA.GAM_temp_df, "results/temporal model/quahog/Delta.GAM_YEAR_Indices.csv", row.names = FALSE)

save(DELTA.S1.GAM_temp_GBK, full_DELTA.GAM_df, file = "results/temporal model/quahog/models objects/DELTA.s1.GAM_temp_GBK.Rdata")
save(DELTA.S2.GAM_temp_GBK, full_DELTA.GAM_df, file = "results/temporal model/quahog/models objects/DELTA.s2.GAM_temp_GBK.Rdata")
save(DELTA.S1.GAM_temp_SVAtoSNE, full_DELTA.GAM_df, file = "results/temporal model/quahog/models objects/DELTA.s1.GAM_temp_SVAtoSNE.Rdata")
save(DELTA.S2.GAM_temp_SVAtoSNE, full_DELTA.GAM_df, file = "results/temporal model/quahog/models objects/DELTA.s2.GAM_temp_SVAtoSNE.Rdata")
...
write.csv(RF_temp_df, "results/temporal model/quahog/RF_YEAR_Indices.csv", row.names = FALSE)

save(RF_temp_GBK, full_RF_df, file = "results/temporal model/quahog/models objects/RF_temp_GBK.Rdata")
save(RF_temp_SVAtoSNE, full_RF_df, file = "results/temporal model/quahog/models objects/RF_temp_SVAtoSNE.Rdata")
...
save(BART.GAM_fit_year_GBK, BART_comp_GBK_dataset_df, file = "results/temporal model/quahog/models objects/BART_year_effect_GAM_GBK.Rdata")
...
save(BART.GAM_fit_year_SVAtoSNE, BART_comp_SVAtoSNE_dataset_df, file = "results/temporal model/quahog/models objects/BART_year_effect_GAM_SVAtoSNE.Rdata")
...
load("results/spatial model/quahog/models objects/BART_presence_outside.Rdata")
load("results/spatial model/quahog/models objects/BART_BIOMASS_outside.Rdata")
...
save(BART.GAM_fit_year_annual, BART_comp_annual_dataset_df, file = "results/temporal model/quahog/models objects/BART_year_effect_GAM_annual.Rdata")
...
write.csv(BART_temp_df, "results/temporal model/quahog/BART_YEAR_Indices.csv", row.names = FALSE)

save(BART_presence_GBK, GBK_BART_df, file = "results/temporal model/quahog/models objects/BART_presence_GBK.Rdata")
save(BART_BIOMASS_GBK, GBK_BART_df, file = "results/temporal model/quahog/models objects/BART_BIOMASS_GBK.Rdata")
save(BART_presence_SVAtoSNE, SVAtoSNE_BART_df, file = "results/temporal model/quahog/models objects/BART_presence_SVAtoSNE.Rdata")
save(BART_BIOMASS_SVAtoSNE, SVAtoSNE_BART_df, file = "results/temporal model/quahog/models objects/BART_BIOMASS_SVAtoSNE.Rdata")
...
for (i in 1:nrow(scenario_df)) {
  ...
  VAST_temp_GBK <- tryCatch(
    # attempt to fit the model
    {
      VAST_temp_GBK <- fit_model(settings = settings,
                                  ...
                                  working_dir = "results/temporal model/quahog/models objects/VAST/")
  ...
  save(VAST_temp_GBK, file = paste0("results/temporal model/quahog/models objects/VAST/VAST_GBK_",
...
write.csv(scenario_df, "results/temporal model/quahog/models objects/VAST/VAST_GBK_model_selection.csv")
...
for (i in 1:nrow(scenario_df)) {
  ...
  save(VAST_temp_SVAtoSNE, file = paste0("results/temporal model/quahog/models objects/VAST/VAST_temp_SVAtoSNE_",
                                       scenario_df$error.dist[i], ".",scenario_df$link.func[i], "_model.Rdata"))
...
write.csv(scenario_df, "results/temporal model/quahog/models objects/VAST/VAST_SVAtoSNE_model_selection.csv")
...
VAST_temp_GBK_df <- data.frame(YEAR = unique(GBK_VAST_df$YEAR),
  ...
  ## SVAtoSNE
load("results/temporal model/quahog/models objects/VAST_temp_SVAtoSNE.Rdata")

VAST_SVAtoSNE_data <- plot(VAST_temp_SVAtoSNE, working_dir = "results/temporal model/quahog/models objects/VAST/figure/")
...
load("results/spatial model/quahog/models objects/VAST_outside_model.Rdata")

VAST_all_data <- plot(VAST_outside_model, working_dir = "results/temporal model/quahog/models objects/VAST/figure/")
...
write.csv(VAST_temp_df, "results/temporal model/quahog/VAST_YEAR_Indices.csv", row.names = FALSE)
...
write.csv(DELTA.GAM_temp_df, "results/temporal model/quahog/simple.Delta.GAM_YEAR_Indices.csv", row.names = FALSE)
