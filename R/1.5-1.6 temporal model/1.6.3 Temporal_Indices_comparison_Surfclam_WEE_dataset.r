...
FD_indices_df <- read.csv("results/stratified.mean.indices/surfclam/original.biomass.indices.csv") %>%
...
OD_indices_df <- read.csv("results/stratified.mean.indices/surfclam/impacted.biomass.indices.csv") %>%
...
write.csv(model_based_df, "results/temporal model/surfclam/model_based_indices.csv", row.names = FALSE)
...
write.csv(design_based_indices_df, "results/temporal model/surfclam/design_based_indices.csv", row.names = FALSE)

  ## 0.1 design-based indices ----
...
png("plots/temporal model/surfclam/design_based_index.png",  width = 8, height = 6, units = 'in', res = 800)
print(design_based_p)
dev.off()

  ## 0.2 model-based indices ----
...
png("plots/temporal model/surfclam/model_based_index.png",  width = 4, height = 8, units = 'in', res = 800)
print(model_based_p)
dev.off()
...
# non_FD_df <- rbind(subset(OD_indices_df, YEAR >= 2002), model_based_df) 
...
write.csv(cor_df, "results/temporal model/surfclam/correlation.df.csv", row.names = FALSE)
...
OD_CV_df <- read.csv("results/stratified.mean.indices/surfclam/impacted.biomass.indices.csv") %>%
  filter(REGION == "SVAtoSNE") %>%
...
write.csv(CV_df, "results/temporal model/surfclam/CV.df.csv", row.names = FALSE)
...
OD_RE_df <- OD_indices_df %>%
  filter(YEAR %in% model_based_df$YEAR) %>% # trim to the model-based time range
...
write.csv(RE_df, "results/temporal model/surfclam/RE.df.csv", row.names = FALSE)
...
write.csv(trend_bias_df, "results/temporal model/surfclam/trend_bias_df.csv", row.names = FALSE)
