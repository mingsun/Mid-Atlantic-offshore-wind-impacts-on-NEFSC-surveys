...
FD_indices_df <- read.csv("results/stratified.mean.indices/longfin.squid/original.indices.csv") %>%
...
OD_indices_df <- read.csv("results/stratified.mean.indices/longfin.squid/impacted.indices.csv") %>%
  add_column(MODEL = "Design-based outside dataset") %>%
  rename(fit = STRATIFIED_MEAN_N, lwr = lo_CI_95, upr = up_CI_95) %>%
  select(YEAR, SEASON, MODEL, fit, lwr, upr)

# random_indices_df <- read.csv("results/stratified.mean.indices/longfin.squid/random.reduction.indices.csv")
...
write.csv(model_based_df, "results/temporal model/longfin.squid/model_based_indices.csv", row.names = FALSE)
...
write.csv(design_based_indices_df, "results/temporal model/longfin.squid/design_based_indices.csv", row.names = FALSE)
...
design_based_p <- ggplot(design_based_indices_df, aes(x = YEAR)) +
  ...
  ylab("biomass indices [kg/tow]") +
  theme_minimal() +
  theme(legend.position = c(0.05, 0.95), # Place the legend in the top-left corner
        legend.justification = c(0, 1))
   
png("plots/temporal model/longfin.squid/design_based_index.png",  width = 8, height = 6, units = 'in', res = 800)
...
png("plots/temporal model/longfin.squid/model_based_index.png",  width = 10, height = 10, units = 'in', res = 800)
print(model_based_p)
dev.off()
...
}; remove(temp_ind, temp_ref)

cor_df <- cor_df %>%
  mutate(SCENARIO = factor(SCENARIO, levels = c("FALL", "SPRING", "ANNUAL-FALL", "ANNUAL-SPRING"))) %>%
  group_by(MODEL) %>%
  mutate(MEAN_COR = mean(Spearman_Cor, na.rm = TRUE)) %>%
  ungroup()

write.csv(cor_df, "results/temporal model/longfin.squid/correlation.df.csv", row.names = FALSE)
...
OD_CV_df <- read.csv("results/stratified.mean.indices/longfin.squid/impacted.indices.csv") %>%
...
write.csv(CV_df, "results/temporal model/longfin.squid/CV.df.csv", row.names = FALSE)
...
write.csv(RE_df, "results/temporal model/longfin.squid/RE.df.csv", row.names = FALSE)
...
trend_bias_df <- rbind(trend_bias_df, band_df)
...
write.csv(trend_bias_df, "results/temporal model/longfin.squid/trend_bias_df.csv", row.names = FALSE)
