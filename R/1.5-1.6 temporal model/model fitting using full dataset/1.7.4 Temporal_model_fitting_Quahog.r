...
catch_by_tow_df <- read.csv("results/stratified.mean.indices/quahog/total.catch.by.tow.csv") %>%
  select(ID, REGION)

full_df <- read.csv("results/stratified.mean.indices/quahog/full.info.tow.list.csv")[,-1] %>%
...
write.csv(TW.GAM_temp_df, "results/temporal model/quahog/full dataset/Tweedie.GAM_YEAR_Indices.csv", row.names = FALSE)

save(TW.GAM_temp_GBK, full_WG_df, file = "results/temporal model/quahog/models objects/full dataset/TW.GAM_temp_GBK.Rdata")
save(TW.GAM_temp_SVAtoSNE, full_WG_df, file = "results/temporal model/quahog/models objects/full dataset/TW.GAM_temp_SVAtoSNE.Rdata")
...
# load(file = "results/temporal model/quahog/models objects/DELTA.s1.GAM_temp_GBK.Rdata")
# load(file = "results/temporal model/quahog/models objects/DELTA.s2.GAM_temp_GBK.Rdata")
...
save(DELTA.GAM_fit_year_GBK, DELTA.GAM_PREDICTED_GBK, file = "results/temporal model/quahog/models objects/full dataset/DELTA.GAM_year_effect_GAM_GBK.Rdata")
# load("results/temporal model/quahog/models objects/DELTA.GAM_year_effect_GAM_GBK.Rdata")
...
# load(file = "results/temporal model/quahog/models objects/DELTA.s1.GAM_temp_SVAtoSNE.Rdata")
# load(file = "results/temporal model/quahog/models objects/DELTA.s2.GAM_temp_SVAtoSNE.Rdata")
...
save(DELTA.GAM_fit_year_SVAtoSNE, DELTA.GAM_PREDICTED_SVAtoSNE, file = "results/temporal model/quahog/models objects/full dataset/DELTA.GAM_year_effect_GAM_SVAtoSNE.Rdata")
# load("results/temporal model/quahog/models objects/DELTA.GAM_year_effect_GAM_SVAtoSNE.Rdata")
...
load("results/spatial model/quahog/models objects/DELTA.S1.GAM_dist_full.Rdata")
load("results/spatial model/quahog/models objects/DELTA.S2.GAM_dist_full.Rdata")
...
save(DELTA.GAM_fit_year_annual, DELTA.GAM_PREDICTED_annual, file = "results/temporal model/quahog/models objects/full dataset/DELTA.GAM_year_effect_GAM_annual.Rdata")
# load("results/temporal model/quahog/models objects/DELTA.GAM_year_effect_GAM_annual.Rdata")
...
write.csv(DELTA.GAM_temp_df, "results/temporal model/quahog/full dataset/Delta.GAM_YEAR_Indices.csv", row.names = FALSE)

save(DELTA.S1.GAM_temp_GBK, full_DELTA.GAM_df, file = "results/temporal model/quahog/models objects/full dataset/DELTA.s1.GAM_temp_GBK.Rdata")
save(DELTA.S2.GAM_temp_GBK, full_DELTA.GAM_df, file = "results/temporal model/quahog/models objects/full dataset/DELTA.s2.GAM_temp_GBK.Rdata")
save(DELTA.S1.GAM_temp_SVAtoSNE, full_DELTA.GAM_df, file = "results/temporal model/quahog/models objects/full dataset/DELTA.s1.GAM_temp_SVAtoSNE.Rdata")
save(DELTA.S2.GAM_temp_SVAtoSNE, full_DELTA.GAM_df, file = "results/temporal model/quahog/models objects/full dataset/DELTA.s2.GAM_temp_SVAtoSNE.Rdata")
...
write.csv(RF_temp_df, "results/temporal model/quahog/full dataset/RF_YEAR_Indices.csv", row.names = FALSE)

save(RF_temp_GBK, full_RF_df, file = "results/temporal model/quahog/models objects/full dataset/RF_temp_GBK.Rdata")
save(RF_temp_SVAtoSNE, full_RF_df, file = "results/temporal model/quahog/models objects/full dataset/RF_temp_SVAtoSNE.Rdata")
...
BART_comp_GBK_dataset_df <- GBK_BART_df %>%
  select(OWF, YEAR, LAT, TEMP, DEPTH, PRESENCE, BIOMASS) %>%
  add_column(SPECIES = "QUAHOG", MODEL = "Bayesian Additive Regression Trees", DATASET = "GBK") %>%
...
AIC(BART.GAM_fit_year_GBK) # gamma = NA, tw = -737.7805, gaussian = NA

save(BART.GAM_fit_year_GBK, BART_comp_GBK_dataset_df, file = "results/temporal model/quahog/models objects/full dataset/BART_year_effect_GAM_GBK.Rdata")
# load("results/temporal model/quahog/models objects/BART_year_effect_GAM_GBK.Rdata")
...
BART_comp_SVAtoSNE_dataset_df <- SVAtoSNE_BART_df %>%
  select(OWF, YEAR, LAT, TEMP, DEPTH, PRESENCE, BIOMASS) %>%
  add_column(SPECIES = "QUAHOG", MODEL = "Bayesian Additive Regression Trees", DATASET = "SVAtoSNE") %>%
...
AIC(BART.GAM_fit_year_SVAtoSNE) # gamma = NA, tw = -3197.039, gaussian = NA

save(BART.GAM_fit_year_SVAtoSNE, BART_comp_SVAtoSNE_dataset_df, file = "results/temporal model/quahog/models objects/full dataset/BART_year_effect_GAM_SVAtoSNE.Rdata")
# load("results/temporal model/quahog/models objects/BART_year_effect_GAM_SVAtoSNE.Rdata")
...
load("results/spatial model/quahog/models objects/BART_presence_full.Rdata")
load("results/spatial model/quahog/models objects/BART_BIOMASS_full.Rdata")
...
#### presence 
BART_presence_annual_predict <- dbarts:::predict.bart(BART_presence_full, newdata = full_BART_df[, c("YEAR", "LAT", "TEMP", "DEPTH")], type = "bart")
...
BART_biomass_annual_predict <- dbarts:::predict.bart(BART_BIOMASS_full , newdata = temp_presence_df[, c("YEAR", "LAT", "TEMP", "DEPTH")], type = "bart")
...
BART_comp_annual_dataset_df <- full_BART_df %>%
  select(OWF, YEAR, LAT, TEMP, DEPTH, PRESENCE, BIOMASS) %>%
  add_column(SPECIES = "QUAHOG", MODEL = "Bayesian Additive Regression Trees", DATASET = "ANNUAL") %>%
...
AIC(BART.GAM_fit_year_annual) # gamma = NA, tw = -3330.948, gaussian = NA

save(BART.GAM_fit_year_annual, BART_comp_annual_dataset_df, file = "results/temporal model/quahog/models objects/full dataset/BART_year_effect_GAM_annual.Rdata")
# load("results/temporal model/quahog/models objects/BART_year_effect_GAM_annual.Rdata")
...
write.csv(BART_temp_df, "results/temporal model/quahog/full dataset/BART_YEAR_Indices.csv", row.names = FALSE)

save(BART_presence_GBK, GBK_BART_df, file = "results/temporal model/quahog/models objects/full dataset/BART_presence_GBK.Rdata")
save(BART_BIOMASS_GBK, GBK_BART_df, file = "results/temporal model/quahog/models objects/full dataset/BART_BIOMASS_GBK.Rdata")
save(BART_presence_SVAtoSNE, SVAtoSNE_BART_df, file = "results/temporal model/quahog/models objects/full dataset/BART_presence_SVAtoSNE.Rdata")
save(BART_BIOMASS_SVAtoSNE, SVAtoSNE_BART_df, file = "results/temporal model/quahog/models objects/full dataset/BART_BIOMASS_SVAtoSNE.Rdata")
...
settings <-  make_settings(n_x = c(100, 250, 500, 1000, 2000)[2], # number of knots, the more knots the longer it runs
                           ...
                           # epsilons are turned off for this scenario
...
for (i in 1:nrow(scenario_df)) {
  ...
  extrapolation_region <- make_extrapolation_info(Region = "northwest_atlantic",  # use the embedded dataset
                                                  ...
                                                  DirPath = "results/temporal model/quahog/models objects/VAST/")
  ...
  VAST_temp_GBK <- tryCatch(
    # attempt to fit the model
    {
      VAST_temp_GBK <- fit_model(settings = settings,
                                  ...
                                  working_dir = "results/temporal model/quahog/models objects/full dataset/VAST/")
  ...
  save(VAST_temp_GBK, file = paste0("results/temporal model/quahog/models objects/full dataset/VAST/VAST_GBK_",
                                     scenario_df$error.dist[i], ".",scenario_df$link.func[i], "_model.Rdata"))
  
  # save(VAST_temp_GBK, full_VAST_df, file = paste0("results/temporal model/quahog/models objects/VAST_temp_GBK.Rdata"))
  # load(file = paste0("results/temporal model/quahog/models objects/VAST_temp_GBK.Rdata"))
  
}

write.csv(scenario_df, "results/temporal model/quahog/models objects/full dataset/VAST/VAST_GBK_model_selection.csv")

### save the fitted value ----

# load("results/temporal model/quahog/models objects/VAST_temp_GBK.Rdata")

VAST_GBK_data <- plot(VAST_temp_GBK, working_dir = "results/temporal model/quahog/models objects/VAST/figure/")
...
for (i in 1:nrow(scenario_df)) {
  ...
  save(VAST_temp_SVAtoSNE, file = paste0("results/temporal model/quahog/models objects/full dataset/VAST/VAST_VAtoSNE_",
                                       scenario_df$error.dist[i], ".",scenario_df$link.func[i], "_model.Rdata"))
  
  # save(VAST_temp_SVAtoSNE, file = paste0("results/temporal model/quahog/models objects/VAST_temp_SVAtoSNE.Rdata"))
  
  # load(file = paste0("results/temporal model/quahog/models objects/VAST_temp_SVAtoSNE.Rdata"))
}

write.csv(scenario_df, "results/temporal model/quahog/models objects/full dataset/VAST/VAST_SVAtoSNE_model_selection.csv")
...
VAST_temp_GBK_df <- data.frame(YEAR = unique(GBK_VAST_df$YEAR),
  ...
  ## SVAtoSNE
load("results/temporal model/quahog/models objects/full dataset/VAST_SVAtoSNE_model.Rdata")

VAST_SVAtoSNE_data <- plot(VAST_temp_SVAtoSNE, working_dir = "results/temporal model/quahog/models objects/full dataset/VAST/figure/")
...
load("results/spatial model/quahog/models objects/VAST_full_model.Rdata")

VAST_all_data <- plot(VAST_full_model, working_dir = "results/temporal model/quahog/models objects/full dataset/VAST/figure/")
...
write.csv(VAST_temp_df, "results/temporal model/quahog/full dataset/VAST_YEAR_Indices.csv", row.names = FALSE)
...
DELTA.GAM_temp_SVAtoSNE_df <- data.frame(YEAR = as.numeric(as.character(DELTA.s1.GAM_SVAtoSNE_df$fit$YEAR)), 
                                         REGION = "SVAtoSNE",
                                         MODEL = "Delta-GAM",
                                         fit = DELTA.s1.GAM_SVAtoSNE_df$fit$visregFit * DELTA.s2.GAM_SVAtoSNE_df$fit$visregFit, 
                                         lwr = DELTA.s1.GAM_SVAtoSNE_df$fit$visregLwr * DELTA.s2.GAM_SVAtoSNE_df$fit$visregLwr,
                                         upr = DELTA.s1.GAM_SVAtoSNE_df$fit$visregUpr * DELTA.s2.GAM_SVAtoSNE_df$fit$visregUpr)
...
load("results/spatial model/surfclam/models objects/DELTA.S1.GAM_dist_full.Rdata")
load("results/spatial model/surfclam/models objects/DELTA.S2.GAM_dist_full.Rdata")
...
write.csv(DELTA.GAM_temp_df, "results/temporal model/quahog/full dataset/simple.Delta.GAM_YEAR_Indices.csv", row.names = FALSE)
