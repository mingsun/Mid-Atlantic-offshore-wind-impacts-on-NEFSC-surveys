...
full_df <- read.csv("results/stratified.mean.indices/longfin.squid/full.info.tow.list.csv")[,-1] 
...
load("results/spatial model/longfin.squid/models objects/TW.GAM_dist_full.Rdata")

TW.GAM_ANNUAL_YEAR_df <- visreg::visreg(TW.GAM_dist_full, data = full_WG_df, "YEAR", scale = "response",  plot = FALSE)
...
write.csv(TW.GAM_temp_df, "results/temporal model/longfin.squid/full dataset/Tweedie.GAM_YEAR_Indices.csv", row.names = FALSE)

save(TW.GAM_temp_fall, full_WG_df, file = "results/temporal model/longfin.squid/models objects/full dataset/TW.GAM_temp_fall.Rdata")
save(TW.GAM_temp_spring, full_WG_df, file = "results/temporal model/longfin.squid/models objects/full dataset/TW.GAM_temp_spring.Rdata")
...
AIC(DELTA.GAM_fit_year_fall) # gamma = 277740.6, tw = NA, gaussian = -39189.77

save(DELTA.GAM_fit_year_fall, DELTA.GAM_PREDICTED_fall, file = "results/temporal model/longfin.squid/models objects/full dataset/DELTA.GAM_year_effect_GAM_fall.Rdata")
# load("results/temporal model/longfin.squid/models objects/DELTA.GAM_year_effect_GAM_fall.Rdata")
...
DELTA.GAM_PREDICTED_spring <- full_DELTA.GAM_df %>%
  ...
  mutate(PREDICTED_PRESENCE = predict(DELTA.S1.GAM_temp_spring, newdata = ., type = "response"),
    ...
    ### generate GAM model based on final prediction for year effect - tw(), Gamma(link = "log"), or gaussian(link = "log")
DELTA.GAM_fit_year_spring <- gam(PREDICTED_BIOMASS ~ YEAR + s(LAT) + s(BOTTEMP) + s(AVGDEPTH), data = DELTA.GAM_PREDICTED_spring, family = gaussian(link = "log"), 
                                 nthreads = parallel::detectCores()) 

AIC(DELTA.GAM_fit_year_spring) # gamma = -149303, tw = NA, gaussian = -163824.7

save(DELTA.GAM_fit_year_spring, DELTA.GAM_PREDICTED_spring, file = "results/temporal model/longfin.squid/models objects/full dataset/DELTA.GAM_year_effect_GAM_spring.Rdata")
# load("results/temporal model/longfin.squid/models objects/DELTA.GAM_year_effect_GAM_spring.Rdata")
...
load("results/spatial model/longfin.squid/models objects/DELTA.S1.GAM_dist_full.Rdata")
load("results/spatial model/longfin.squid/models objects/DELTA.S2.GAM_dist_full.Rdata")
...
AIC(DELTA.GAM_fit_year_annual) # gamma = 349564, tw = -36868.84, gaussian = -54080.76

save(DELTA.GAM_fit_year_annual, DELTA.GAM_PREDICTED_annual, file = "results/temporal model/longfin.squid//models objects/full dataset/DELTA.GAM_year_effect_GAM_annual.Rdata")
...
write.csv(DELTA.GAM_temp_df, "results/temporal model/longfin.squid/full dataset/Delta.GAM_YEAR_Indices.csv", row.names = FALSE)

save(DELTA.S1.GAM_temp_fall, full_DELTA.GAM_df, file = "results/temporal model/longfin.squid/models objects/full dataset/DELTA.s1.GAM_temp_fall.Rdata")
save(DELTA.S2.GAM_temp_fall, full_DELTA.GAM_df, file = "results/temporal model/longfin.squid/models objects/full dataset/DELTA.s2.GAM_temp_fall.Rdata")
save(DELTA.S1.GAM_temp_spring, full_DELTA.GAM_df, file = "results/temporal model/longfin.squid/models objects/full dataset/DELTA.s1.GAM_temp_spring.Rdata")
save(DELTA.S2.GAM_temp_spring, full_DELTA.GAM_df, file = "results/temporal model/longfin.squid/models objects/full dataset/DELTA.s2.GAM_temp_spring.Rdata")
...
load("results/spatial model/longfin.squid/models objects/RF_dist_full.Rdata")

RF_annual_df <- partial(RF_dist_full, pred.var = "YEAR", grid.resolution = 100, plot = FALSE, train = full_RF_df)
...
write.csv(RF_temp_df, "results/temporal model/longfin.squid/full dataset/RF_YEAR_Indices.csv", row.names = FALSE)

save(RF_temp_fall, full_RF_df, file = "results/temporal model/longfin.squid/models objects/full dataset/RF_temp_fall.Rdata")
save(RF_temp_spring, full_RF_df, file = "results/temporal model/longfin.squid/models objects/full dataset/RF_temp_spring.Rdata")
...
BART_comp_fall_dataset_df <- fall_BART_df %>%
  select(OWF, YEAR, LAT, BOTTEMP, AVGDEPTH, PRESENCE, BIOMASS) %>%
  add_column(SPECIES = "LONGFIN SQUID", MODEL = "Bayesian Additive Regression Trees", DATASET = "FALL") %>%
...
BART.GAM_fit_year_fall <- gam(PREDICTED_BIOMASS ~ YEAR + s(LAT) + s(BOTTEMP) + s(AVGDEPTH), data = BART_comp_fall_dataset_df, family = tw(), 
                              nthreads = parallel::detectCores()) 

AIC(BART.GAM_fit_year_fall) # gamma = NA, tw = 35177.15, gaussian = 35934.93

save(BART.GAM_fit_year_fall, BART_comp_fall_dataset_df, file = "results/temporal model/longfin.squid/models objects/full dataset/BART_year_effect_GAM_fall.Rdata")
# load("results/temporal model/longfin.squid/models objects/BART_year_effect_GAM_fall.Rdata")
...
BART_comp_spring_dataset_df <- spring_BART_df %>%
  select(OWF, YEAR, LAT, BOTTEMP, AVGDEPTH, PRESENCE, BIOMASS) %>%
  add_column(SPECIES = "LONGFIN SQUID", MODEL = "Bayesian Additive Regression Trees", DATASET = "SPRING") %>%
  mutate(PREDICTED_PRESENCE = BART_presence_spring_predict) %>%
  mutate(PREDICTED_PRESENCE = ifelse(PREDICTED_PRESENCE >= cutoff * 0.99, 1, 0)) %>%  # use cutoff to categorize probability into 0 and 1
  left_join(temp_presence_df) %>%
  mutate(PREDICTED_BIOMASS = ifelse(is.na(PREDICTED_BIOMASS), 0, PREDICTED_BIOMASS)) %>%  # fill in the absent data with 0
  mutate(DELTA_PREDICTION = PREDICTED_PRESENCE * PREDICTED_BIOMASS) %>%
  mutate(DELTA_PREDICTION = ifelse(DELTA_PREDICTION < 0, 0, DELTA_PREDICTION)) %>% # post-processing to avoid negative BIOMASS
...
AIC(BART.GAM_fit_year_spring) # gamma = NA, tw = 18889.96, gaussian = NA

save(BART.GAM_fit_year_spring, BART_comp_spring_dataset_df, file = "results/temporal model/longfin.squid/models objects/full dataset/BART_year_effect_GAM_spring.Rdata")
# load("results/temporal model/longfin.squid/models objects/BART_year_effect_GAM_spring.Rdata")
...
load("results/spatial model/longfin.squid/models objects/BART_presence_full.Rdata")
load("results/spatial model/longfin.squid/models objects/BART_BIOMASS_full.Rdata")

invisible(BART_presence_full$fit$state)
invisible(BART_BIOMASS_full$fit$state) 

cutoff <- InformationValue::optimalCutoff(full_BART_df$PRESENCE, fitted(BART_presence_full)) 


### 4.3.1 predict for the entire region dataset ----

#### presence 
BART_presence_annual_predict <- dbarts:::predict.bart(BART_presence_full, newdata = full_BART_df[, c("YEAR", "LAT", "BOTTEMP", "AVGDEPTH")], type = "bart")
...
BART_biomass_annual_predict <- dbarts:::predict.bart(BART_BIOMASS_full , newdata = temp_presence_df[, c("YEAR", "LAT", "BOTTEMP", "AVGDEPTH")], type = "bart")
...
BART_comp_annual_dataset_df <- full_BART_df %>%
  select(OWF, YEAR, LAT, BOTTEMP, AVGDEPTH, PRESENCE, BIOMASS) %>%
  add_column(SPECIES = "LONGFIN SQUID", MODEL = "Bayesian Additive Regression Trees", DATASET = "ANNUAL") %>%
...
AIC(BART.GAM_fit_year_annual) # gamma = NA, tw = 52717.98, gaussian = NA

save(BART.GAM_fit_year_annual, BART_comp_annual_dataset_df, file = "results/temporal model/longfin.squid/models objects/full dataset/BART_year_effect_GAM_annual.Rdata")
# load("results/temporal model/longfin.squid/models objects/BART_year_effect_GAM_annual.Rdata")
...
write.csv(BART_temp_df, "results/temporal model/longfin.squid/full dataset/BART_YEAR_Indices.csv", row.names = FALSE)

save(BART_presence_fall, fall_BART_df, file = "results/temporal model/longfin.squid/models objects/full dataset/BART_presence_fall.Rdata")
save(BART_BIOMASS_fall, fall_BART_df, file = "results/temporal model/longfin.squid/models objects/full dataset/BART_BIOMASS_fall.Rdata")
save(BART_presence_spring, spring_BART_df, file = "results/temporal model/longfin.squid/models objects/full dataset/BART_presence_spring.Rdata")
save(BART_BIOMASS_spring, spring_BART_df, file = "results/temporal model/longfin.squid/models objects/full dataset/BART_BIOMASS_spring.Rdata")
...
for (i in 1:nrow(scenario_df)) {
  ...
  VAST_temp_fall <- tryCatch(
    # attempt to fit the model
    {
      VAST_temp_fall <- fit_model(settings = settings,
                                  ...
                                  working_dir = "results/temporal model/longfin.squid/models objects/full dataset/VAST/")
  ...
  save(VAST_temp_fall, file = paste0("results/temporal model/longfin.squid/models objects/full dataset/VAST_fall_",
...
write.csv(scenario_df, "results/temporal model/longfin.squid/models objects/full dataset/VAST/VAST_fall_model_selection.csv")

 ### save the fitted value ----

load("results/temporal model/longfin.squid/models objects/full dataset/VAST_fall_model.Rdata")

VAST_fall_data <- plot(VAST_temp_fall, working_dir = "results/temporal model/longfin.squid/models objects/full dataset/VAST/figure/")
...
# if(exists("VAST_temp_fall") == FALSE) {
#   VAST_temp_fall_df <- data.frame(YEAR = unique(full_VAST_df$Year),
#                                   Season = "FALL",
#                                   MODEL = "VAST",
#                                   fit = NA, 
#                                   lwr = NA, 
...
for (i in 2:nrow(scenario_df)) {
  ...
  save(VAST_temp_spring, file = paste0("results/temporal model/longfin.squid/models objects/full dataset/VAST_spring_",
                                                   scenario_df$error.dist[i], ".",scenario_df$link.func[i], "_model.Rdata"))
...
write.csv(scenario_df, "results/temporal model/longfin.squid/models objects/full dataset/VAST/VAST_spring_model_selection.csv")


### save the fitted value ----

load("results/temporal model/longfin.squid/models objects/full dataset/VAST_spring_model.Rdata")

VAST_spring_data <- plot(VAST_temp_spring, working_dir = "results/temporal model/longfin.squid/models objects/full dataset/VAST/figure/")
...
# if(exists("VAST_temp_spring") == FALSE) {
#   VAST_temp_spring_df <- data.frame(YEAR = unique(full_VAST_df$Year),
#                                   Season = "SPRING",
#                                   MODEL = "VAST",
#                                   fit = NA, 
#                                   lwr = NA, 
...
load("results/spatial model/longfin.squid/models objects/VAST_full_model.Rdata")

VAST_annual_data <- plot(VAST_full_model, working_dir = "results/temporal model/longfin.squid/models objects/full dataset/VAST/figure/")
...
write.csv(VAST_temp_df, "results/temporal model/longfin.squid/full dataset/VAST_YEAR_Indices.csv", row.names = FALSE)
...
### generate 2-stage predicted values
DELTA.s1.GAM_ANNUAL_df <- visreg::visreg(DELTA.S1.GAM_dist_full, data = full_DELTA.GAM_df, "YEAR", scale = "response",  plot = FALSE)
DELTA.s2.GAM_ANNUAL_df <- visreg::visreg(DELTA.S2.GAM_dist_full, data = subset(full_DELTA.GAM_df, PRESENCE == 1), "YEAR", scale = "response", plot = FALSE)
...
## combine and save ----
DELTA.GAM_temp_df <- rbind(DELTA.GAM_temp_fall_df, DELTA.GAM_temp_spring_df, DELTA.GAM_temp_annual_df) 
write.csv(DELTA.GAM_temp_df, "results/temporal model/longfin.squid/full dataset/simple.Delta.GAM_YEAR_Indices.csv", row.names = FALSE)
