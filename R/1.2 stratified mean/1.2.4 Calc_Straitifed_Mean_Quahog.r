...
tow_1982_df <- read.csv("data/NOAA.stock.data/quahog/TOW_DATA.CSV") %>% 
...
tow_1997_df <- read.csv("data/NOAA.stock.data/quahog/lengths/TOW_DATA.CSV") %>% 
  select(CRUISE6, YR, STATION, STRATUM, TOW, LENGRP, NPERTOW, KGPERTOW, ASWEPT_M2, LAT, LON, DEPTH, TEMP) %>%
  rename(YEAR = YR) %>%
  filter(YEAR >= 1997) %>%
  filter(nchar(TOW) != 9) %>% # remove the repeated tows, looks like the data are already filled in to the original record
...
write.csv(tow_df, "results/stratified.mean.indices/quahog/tow.list.csv", row.names = FALSE) # this is supposed to be tow header only, but I saved everything here
...
SOUTH.STRATA <- paste0(1:6, "Q")
NORTH.STRATA <- paste0(7:12, "Q")

stra_area_df <- read.csv("data/NOAA.stock.data/quahog/STRATA_MEANS.CSV") %>%
  select(STRATUM, AREASQNM) %>%
  mutate(REGION = ifelse(STRATUM %in% SOUTH.STRATA, "SVAtoSNE", "GBK")) %>%
  distinct()

write.csv(stra_area_df, "results/stratified.mean.indices/quahog/strata.area.csv", row.names = FALSE) # biomass/abundance both included
...
write.csv(catch_by_tow_df, "results/stratified.mean.indices/quahog/total.catch.by.tow.csv", row.names = FALSE) # biomass/abundance both included
...
write.csv(stratified_mean_N_df, "results/stratified.mean.indices/quahog/original.indices.csv", row.names = FALSE)
...
write.csv(stratified_mean_N_df, "results/stratified.mean.indices/quahog/original.biomass.indices.csv", row.names = FALSE)
...
write.csv(impacted_stratified_mean_N_df, "results/stratified.mean.indices/quahog/impacted.indices.csv", row.names = FALSE)
...
write.csv(impacted_stratified_mean_N_df, "results/stratified.mean.indices/quahog/impacted.biomass.indices.csv", row.names = FALSE)
