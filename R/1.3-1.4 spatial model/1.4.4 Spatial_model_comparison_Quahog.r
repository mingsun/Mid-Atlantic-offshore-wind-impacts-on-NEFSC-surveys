...
TW.GAM_comp <- read.csv("results/spatial model/quahog/Tweedie.GAM_abundance_comparison.csv")
DELTA.GAM_comp <- read.csv("results/spatial model/quahog/Delta.GAM_abundance_comparison.csv")
RF_comp <- read.csv("results/spatial model/quahog/RF_abundance_comparison.csv")
BART_comp <- read.csv("results/spatial model/quahog/BART_abundance_comparison.csv")
VAST_comp <- read.csv("results/spatial model/quahog/VAST_abundance_comparison.csv"); colnames(VAST_comp)[2:3] <- c("YEAR", "LAT")
...
write.csv(Pred_df, "results/spatial model/quahog/Pred_df.csv")
...
write.csv(RMSE_diff_df, "results/spatial model/quahog/RMSE_diff_df.csv")
...
png("plots/spatial model/quahog/RMSE_difference_between_FD_OD.png",  width = 8, height = 8, units = 'in', res = 800)
print(RMSE_diff_p)
dev.off()
...
write.csv(CV_diff_df, "results/spatial model/quahog/CV_diff_df.csv")
...
png("plots/spatial model/quahog/CV_difference_between_FD_OD.png",  width = 8, height = 8, units = 'in', res = 800)
print(CV_diff_plot)
dev.off()
...
png("plots/spatial model/quahog/Taylor_Diagram_both.png",  width = 8, height = 8, units = 'in', res = 800)
print(TD_p)
dev.off()

TD_FD_p <- TaylorDiagram(subset(Pred_df, DATASET == "FULL"), obs = "BIOMASS", mod = "PREDICTED_BIOMASS", normalise = TRUE, group = "MODEL")

png("plots/spatial model/quahog/Taylor_Diagram_FD.png",  width = 5, height = 5, units = 'in', res = 800)
print(TD_FD_p)
dev.off()
...
png("plots/spatial model/quahog/Taylor_Diagram_OD.png",  width = 5, height = 5, units = 'in', res = 800)
print(TD_OD_p)
dev.off()
