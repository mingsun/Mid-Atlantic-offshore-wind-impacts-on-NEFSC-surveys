...
tow_list_df <- read.csv("results/stratified.mean.indices/longfin.squid/tow.list.csv") # the tows used in the longfin.squid assessment
...
catch_by_tow_df <- read.csv("results/stratified.mean.indices/longfin.squid/calibrated.total.biomass.by.tow.csv") [,-c(10, 12:14)] # fit biomass, not abundance
# catch_by_tow_df <- read.csv("results/stratified.mean.indices/longfin.squid/uncalibrated.total.catch.by.tow.csv")[,-c(11:15)]
catch_by_tow_df$BIOMASS <- catch_by_tow_df$CATCH_WT_CAL

full_df <- full_df %>%
  mutate(STRATUM = as.integer(STRATUM)) %>%
  left_join(catch_by_tow_df) %>% # 11240 tows
  # mutate(NUMBER = as.integer(NUMBER)) %>% # convert to integer for possison distrinbution
  na.omit() # 7549 tows
...
write.csv(full_df, "results/stratified.mean.indices/longfin.squid/full.info.tow.list.csv")
...
TW.GAM_comp_full_dataset_df <- full_df %>%
  ...
  add_column(SPECIES = "LONGFIN SQUID", MODEL = "Tweedie-GAM", DATASET = "FULL") %>%
...
TW.GAM_comp_outside_dataset_df <- full_df %>%
  # filter(OWF == "INSIDE") %>%
  select(OWF, YEAR, LAT, BOTTEMP, AVGDEPTH, BIOMASS) %>%
  add_column(SPECIES = "LONGFIN SQUID", MODEL = "Tweedie-GAM", DATASET = "OUTSIDE") %>%
...
write.csv(TW.GAM_comp, "results/spatial model/longfin.squid/Tweedie.GAM_abundance_comparison.csv", row.names = FALSE)

save(TW.GAM_dist_full, full_WG_df, file = "results/spatial model/longfin.squid/models objects/TW.GAM_dist_full.Rdata")
save(TW.GAM_dist_outside, full_WG_df, file = "results/spatial model/longfin.squid/models objects/TW.GAM_dist_outside.Rdata")
...
DELTA.S2.GAM_dist_full <- gam(BIOMASS ~ YEAR + s(LAT) + s(BOTTEMP) + s(AVGDEPTH), family = tw(), data = subset(full_DELTA.GAM_df, PRESENCE == 1))
...
DELTA.GAM_comp_full_dataset_df <- full_DELTA.GAM_df %>%
  ...
  add_column(SPECIES = "LONGFIN SQUID", MODEL = "Delta-GAM", DATASET = "FULL") %>%
...
DELTA.S2.GAM_dist_outside <- gam(BIOMASS ~ YEAR + s(LAT) + s(BOTTEMP) + s(AVGDEPTH), family = tw(), data = subset(full_DELTA.GAM_df, OWF == "OUTSIDE" & PRESENCE == 1))
...
DELTA.GAM_comp_outside_dataset_df <- full_DELTA.GAM_df %>%
  # filter(OWF == "INSIDE") %>%
  select(OWF, YEAR, LAT, BOTTEMP, AVGDEPTH, BIOMASS, PRESENCE) %>%
  add_column(SPECIES = "LONGFIN SQUID", MODEL = "Delta-GAM", DATASET = "OUTSIDE") %>%
...
write.csv(DELTA.GAM_comp, "results/spatial model/longfin.squid/Delta.GAM_abundance_comparison.csv", row.names = FALSE)

save(DELTA.S1.GAM_dist_full, full_DELTA.GAM_df, file = "results/spatial model/longfin.squid/models objects/DELTA.S1.GAM_dist_full.Rdata")
save(DELTA.S2.GAM_dist_full, full_DELTA.GAM_df, file = "results/spatial model/longfin.squid/models objects/DELTA.S2.GAM_dist_full.Rdata")
save(DELTA.S1.GAM_dist_outside, full_DELTA.GAM_df, file = "results/spatial model/longfin.squid/models objects/DELTA.S1.GAM_dist_outside.Rdata")
save(DELTA.S2.GAM_dist_outside, full_DELTA.GAM_df, file = "results/spatial model/longfin.squid/models objects/DELTA.S2.GAM_dist_outside.Rdata")
...
RF_comp_full_dataset_df <- full_RF_df %>%
  # filter(OWF == "INSIDE") %>%
  select(OWF, YEAR, LAT, BOTTEMP, AVGDEPTH, BIOMASS) %>%
  add_column(SPECIES = "LONGFIN SQUID", MODEL = "Random Forest",DATASET = "FULL") %>%
...
RF_comp_outside_dataset_df <- full_RF_df %>%
  # filter(OWF == "INSIDE") %>%
  select(OWF, YEAR, LAT, BOTTEMP, AVGDEPTH, BIOMASS) %>%
  add_column(SPECIES = "LONGFIN SQUID", MODEL = "Random Forest", DATASET = "OUTSIDE") %>%
...
write.csv(RF_comp, "results/spatial model/longfin.squid/RF_abundance_comparison.csv", row.names = FALSE)

save(RF_dist_full, full_RF_df, file = "results/spatial model/longfin.squid/models objects/RF_dist_full.Rdata")
save(RF_dist_outside, full_RF_df, file = "results/spatial model/longfin.squid/models objects/RF_dist_outside.Rdata")
...
BART_comp_full_dataset_df <- full_BART_df %>%
  select(OWF, YEAR, LAT, BOTTEMP, AVGDEPTH, PRESENCE, BIOMASS) %>%
  add_column(SPECIES = "LONGFIN SQUID", MODEL = "Bayesian Additive Regression Trees", DATASET = "FULL") %>%
...
BART_comp_outside_dataset_df <- full_BART_df %>%
  select(OWF, YEAR, LAT, BOTTEMP, AVGDEPTH, PRESENCE, BIOMASS) %>%
  add_column(SPECIES = "LONGFIN SQUID", MODEL = "Bayesian Additive Regression Trees", DATASET = "OUTSIDE") %>%
...
write.csv(BART_comp, "results/spatial model/longfin.squid/BART_abundance_comparison.csv", row.names = FALSE)

save(BART_presence_full, full_BART_df, file = "results/spatial model/longfin.squid/models objects/BART_presence_full.Rdata")
save(BART_BIOMASS_full, full_BART_df, file = "results/spatial model/longfin.squid/models objects/BART_BIOMASS_full.Rdata")
save(BART_presence_outside, outside_BART_df, file = "results/spatial model/longfin.squid/models objects/BART_presence_outside.Rdata")
save(BART_BIOMASS_outside, outside_BART_df, file = "results/spatial model/longfin.squid/models objects/BART_BIOMASS_outside.Rdata")
...
for (i in 1:nrow(scenario_df)) {
  ...
  VAST_full_model <- tryCatch(
    # attempt to fit the model
    {
      VAST_full_model <- fit_model(settings = settings,
                                   ...
                                   working_dir = "results/spatial model/longfin.squid/models objects/VAST/")
  ...
  VAST_comp_full_dataset_df <- full_VAST_df %>%
    ...
    add_column(SPECIES = "LONGFIN SQUID", MODEL = "VAST", DATASET = "FULL") %>%
  ...
  save(VAST_full_model, file = paste0("results/spatial model/longfin.squid/models objects/VAST/VAST_full_",
                                                    scenario_df$error.dist[i], ".",scenario_df$link.func[i], "_model.Rdata"))
  
  # remove(VAST_full_model, VAST_comp_full_dataset_df)
}
  
write.csv(scenario_df, "results/spatial model/longfin.squid/models objects/VAST/VAST_full_model_selection.csv")
...
for (i in 1:nrow(scenario_df)) {
  ...
  VAST_comp_outside_dataset_df <- full_VAST_df %>%
    select(OWF, Year, Lat, Lon, BOTTEMP, AVGDEPTH, AreaSwept_km2, BIOMASS) %>%
    add_column(SPECIES = "LONGFIN SQUID", MODEL = "VAST", DATASET = "OUTSIDE") %>%
  ...
  save(VAST_outside_model, file = paste0("results/spatial model/longfin.squid/models objects/VAST/VAST_outside_",
                                                          scenario_df$error.dist[i], ".",scenario_df$link.func[i], "_model.Rdata"))
  
  # remove(VAST_outside_model, VAST_comp_outside_dataset_df)
}

write.csv(scenario_df, "results/spatial model/longfin.squid/models objects/VAST/VAST_outside_model_selection.csv", row.names = FALSE)
...
write.csv(VAST_comp, "results/spatial model/longfin.squid/VAST_abundance_comparison.csv", row.names = FALSE)
