...
TW.GAM_comp <- read.csv("results/spatial model/longfin.squid/Tweedie.GAM_abundance_comparison.csv")
DELTA.GAM_comp <- read.csv("results/spatial model/longfin.squid/Delta.GAM_abundance_comparison.csv")
RF_comp <- read.csv("results/spatial model/longfin.squid/RF_abundance_comparison.csv")
BART_comp <- read.csv("results/spatial model/longfin.squid/BART_abundance_comparison.csv")
VAST_comp <- read.csv("results/spatial model/longfin.squid/VAST_abundance_comparison.csv"); colnames(VAST_comp)[2:3] <- c("YEAR", "LAT")
...
write.csv(Pred_df, "results/spatial model/longfin.squid/Pred_df.csv")
...
write.csv(RMSE_diff_df, "results/spatial model/longfin.squid/RMSE_diff_df.csv")
...
png("plots/spatial model/longfin.squid/RMSE_difference_between_FD_OD.png",  width = 8, height = 8, units = 'in', res = 800)
print(RMSE_diff_p)
dev.off()
...
sample_df <- read.csv("results/BTS_sample_loss_all_species_by_year.csv") %>%
  filter(SVSPP == 103,      # (summer flounder: 103, surf clam: 403, quahog 409, longfin squid 503)
         YEAR >= 1982) %>%  # longfin.squid assessment uses survey data since 1976
  select(c(3,6)) %>%
...
png("plots/spatial model/longfin.squid/RMSE_effort_relationship.png",  width = 6, height = 9, units = 'in', res = 800)
print(RMSE_effort_p)
dev.off()
...
png("plots/spatial model/longfin.squid/RMSE_sample_relationship.png",  width = 6, height = 9, units = 'in', res = 800)
print(RMSE_sample_p)
dev.off()
...
png("plots/spatial model/longfin.squid/NSE_difference_between_FD_OD.png",  width = 8, height = 8, units = 'in', res = 800)
print(NSE_diff_plot)
dev.off()
...
write.csv(CV_diff_df, "results/spatial model/longfin.squid/CV_diff_df.csv")
...
png("plots/spatial model/longfin.squid/CV_difference_between_FD_OD.png",  width = 8, height = 8, units = 'in', res = 800)
print(CV_diff_plot)
dev.off()
...
png("plots/spatial model/longfin.squid/Taylor_Diagram_both.png",  width = 8, height = 8, units = 'in', res = 800)
print(TD_p)
dev.off()

TD_FD_p <- TaylorDiagram(subset(Pred_df, DATASET == "FULL"), obs = "BIOMASS", mod = "PREDICTED_BIOMASS", normalise = TRUE, group = "MODEL")

png("plots/spatial model/longfin.squid/Taylor_Diagram_FD.png",  width = 5, height = 5, units = 'in', res = 800)
print(TD_FD_p)
dev.off()
...
png("plots/spatial model/longfin.squid/Taylor_Diagram_OD.png",  width = 5, height = 5, units = 'in', res = 800)
print(TD_OD_p)
dev.off()
