...
full_df <- read.csv("results/stratified.mean.indices/quahog/tow.list.csv") %>%
...
full_df <- full_df %>% 
  ...
  filter(!is.na(BIOMASS) & !is.na(YEAR) & !is.na(LAT) & !is.na(TEMP) & !is.na(DEPTH))
...
sum(full_df$BIOMASS == 0)/nrow(full_df) # 0.1231343

write.csv(full_df, "results/stratified.mean.indices/quahog/full.info.tow.list.csv")
...
TW.GAM_comp_full_dataset_df <- full_df %>%
  ...
  add_column(SPECIES = "QUAHOG", MODEL = "Tweedie-GAM", DATASET = "FULL") %>%
...
TW.GAM_comp_outside_dataset_df <- full_df %>%
  # filter(OWF == "INSIDE") %>%
  select(OWF, YEAR, LAT, TEMP, DEPTH, BIOMASS) %>%
  add_column(SPECIES = "QUAHOG", MODEL = "Tweedie-GAM", DATASET = "OUTSIDE") %>%
...
write.csv(TW.GAM_comp, "results/spatial model/quahog/Tweedie.GAM_abundance_comparison.csv", row.names = FALSE)

save(TW.GAM_dist_full, full_WG_df, file = "results/spatial model/quahog/models objects/TW.GAM_dist_full.Rdata")
save(TW.GAM_dist_outside, full_WG_df, file = "results/spatial model/quahog/models objects/TW.GAM_dist_outside.Rdata")
...
DELTA.GAM_comp_full_dataset_df <- full_DELTA.GAM_df %>%
  ...
  add_column(SPECIES = "QUAHOG", MODEL = "Delta-GAM", DATASET = "FULL") %>%
...
DELTA.GAM_comp_outside_dataset_df <- full_DELTA.GAM_df %>%
  # filter(OWF == "INSIDE") %>%
  select(OWF, YEAR, LAT, TEMP, DEPTH, BIOMASS, PRESENCE) %>%
  add_column(SPECIES = "QUAHOG", MODEL = "Delta-GAM", DATASET = "OUTSIDE") %>%
...
write.csv(DELTA.GAM_comp, "results/spatial model/quahog/Delta.GAM_abundance_comparison.csv", row.names = FALSE)

save(DELTA.S1.GAM_dist_full, full_DELTA.GAM_df, file = "results/spatial model/quahog/models objects/DELTA.S1.GAM_dist_full.Rdata")
save(DELTA.S2.GAM_dist_full, full_DELTA.GAM_df, file = "results/spatial model/quahog/models objects/DELTA.S2.GAM_dist_full.Rdata")
save(DELTA.S1.GAM_dist_outside, full_DELTA.GAM_df, file = "results/spatial model/quahog/models objects/DELTA.S1.GAM_dist_outside.Rdata")
save(DELTA.S2.GAM_dist_outside, full_DELTA.GAM_df, file = "results/spatial model/quahog/models objects/DELTA.S2.GAM_dist_outside.Rdata")
...
RF_comp_full_dataset_df <- full_RF_df %>%
  # filter(OWF == "INSIDE") %>%
  select(OWF, YEAR, LAT, TEMP, DEPTH, BIOMASS) %>%
  add_column(SPECIES = "QUAHOG", MODEL = "Random Forest",DATASET = "FULL") %>%
...
RF_comp_outside_dataset_df <- full_RF_df %>%
  # filter(OWF == "INSIDE") %>%
  select(OWF, YEAR, LAT, TEMP, DEPTH, BIOMASS) %>%
  add_column(SPECIES = "QUAHOG", MODEL = "Random Forest", DATASET = "OUTSIDE") %>%
...
write.csv(RF_comp, "results/spatial model/quahog/RF_abundance_comparison.csv", row.names = FALSE)

save(RF_dist_full, full_RF_df, file = "results/spatial model/quahog/models objects/RF_dist_full.Rdata")
save(RF_dist_outside, full_RF_df, file = "results/spatial model/quahog/models objects/RF_dist_outside.Rdata")
...
BART_comp_full_dataset_df <- full_BART_df %>%
  select(OWF, YEAR, LAT, TEMP, DEPTH, PRESENCE, BIOMASS) %>%
  add_column(SPECIES = "QUAHOG", MODEL = "Bayesian Additive Regression Trees", DATASET = "FULL") %>%
...
BART_comp_outside_dataset_df <- full_BART_df %>%
  select(OWF, YEAR, LAT, TEMP, DEPTH, PRESENCE, BIOMASS) %>%
  add_column(SPECIES = "QUAHOG", MODEL = "Bayesian Additive Regression Trees", DATASET = "OUTSIDE") %>%
...
write.csv(BART_comp, "results/spatial model/quahog/BART_abundance_comparison.csv", row.names = FALSE)

save(BART_presence_full, full_BART_df, file = "results/spatial model/quahog/models objects/BART_presence_full.Rdata")
save(BART_BIOMASS_full, full_BART_df, file = "results/spatial model/quahog/models objects/BART_BIOMASS_full.Rdata")
save(BART_presence_outside, outside_BART_df, file = "results/spatial model/quahog/models objects/BART_presence_outside.Rdata")
save(BART_BIOMASS_outside, outside_BART_df, file = "results/spatial model/quahog/models objects/BART_BIOMASS_outside.Rdata")
...
for (i in 1:nrow(scenario_df)) {
  ...
  VAST_full_model <- tryCatch(
    # attempt to fit the model
    {
      VAST_full_model <- fit_model(settings = settings,
                                   ...
                                   working_dir = "results/spatial model/quahog/models objects/VAST/")
  ...
  VAST_comp_full_dataset_df <- full_VAST_df %>%
    ...
    add_column(SPECIES = "QUAHOG", MODEL = "VAST", DATASET = "FULL") %>%
  ...
  save(VAST_full_model, file = paste0("results/spatial model/quahog/models objects/VAST/VAST_full_",
                                                    scenario_df$error.dist[i], ".",scenario_df$link.func[i], "_model.Rdata"))
  
  write.csv(scenario_df, "results/spatial model/quahog/models objects/VAST/VAST_full_model_selection.csv")
...
for (i in 1:nrow(scenario_df)) {
  ...
  VAST_comp_outside_dataset_df <- full_VAST_df %>%
    select(OWF, Year, Lat, Lon, TEMP, DEPTH, AreaSwept_km2, BIOMASS) %>%
    add_column(SPECIES = "QUAHOG", MODEL = "VAST", DATASET = "OUTSIDE") %>%
  ...
  save(VAST_outside_model, file = paste0("results/spatial model/quahog/models objects/VAST/VAST_outside_",
                                                          scenario_df$error.dist[i], ".",scenario_df$link.func[i], "_model.Rdata"))
  
  # remove(VAST_outside_model, VAST_comp_outside_dataset_df)
}

write.csv(scenario_df, "results/spatial model/quahog/models objects/VAST/VAST_outside_model_selection.csv", row.names = FALSE)
...
write.csv(VAST_comp, "results/spatial model/quahog/VAST_abundance_comparison.csv", row.names = FALSE)
