...
load("results/indices for assessment/summer flounder/mitigation_2_model-standardized/MIT.2.ALB_ratio.Rdata")
load("results/indices for assessment/summer flounder/mitigation_2_model-standardized/MIT.2.BIG_ratio.Rdata")
...
BIG_fall <- data$dat$IAA_mats[[26]][28:41,c(2,4:11)]
    ...
    ## 2.3 apply the ratio to estimate Wind Energy Excluded indices (MIT.2) ----

# note: be careful with the round and -999 values, need to be very consistent with the original indices format 

ALB_spring_MIT.2 <- round(ALB_spring * ALB_MIT.2_SPRING_ratio, 2)
ALB_fall_MIT.2 <-   round(ALB_fall *   ALB_MIT.2_FALL_ratio, 2)

BIG_spring_MIT.2 <- round(BIG_spring * BIG_MIT.2_SPRING_ratio)
BIG_spring_MIT.2[c(12),] <- -999

BIG_fall_MIT.2 <-   round(BIG_fall *   BIG_MIT.2_FALL_ratio)
BIG_fall_MIT.2[c(9, 12),] <- -999
...
data$dat$IAA_mats[[2]][1:27,c(2,4:11)] <- ALB_spring_MIT.2

# ALB fall (1982-2008)
data$dat$IAA_mats[[3]][1:27,c(2,4:11)] <- ALB_fall_MIT.2

# BIG spring (2009-2022)
data$dat$IAA_mats[[25]][28:41,c(2,4:11)] <- BIG_spring_MIT.2

# BIG fall (2009-2022)
data$dat$IAA_mats[[26]][28:41,c(2,4:11)] <- BIG_fall_MIT.2

WriteASAP3DatFile("assessment model/summer flounder/ASAP input data/ASAP3_MTA2023_MIT.2_BTS.DAT", data, 
                  header.text = "Summer Flounder 2023: BTS indices using MIT.2 dataset (Sun 2024) \n \n \n ") 
...
wd <- "assessment model/summer flounder/ASAP/MIT.2 model/"
asap.name <- "ASAP3_MTA2023_MIT.2_BTS"
asap <- dget("assessment model/summer flounder/ASAP/MIT.2 model/ASAP3_MTA2023_MIT.2_BTS.RDAT") 
...
output <- read.csv("results/stock assessment/summer flounder/ASAP_summary_ASAP3_MTA2023_MIT.2_BTS.csv") %>%
  add_column(Scenario = "MIT.2 Model") %>%
  write.csv("results/stock assessment/summer flounder/ASAP_summary_ASAP3_MTA2023_MIT.2_BTS.csv", row.names = FALSE)
